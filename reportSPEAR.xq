...
let $descURIs := for $desc in $descs return fn:distinct-values(fn:tokenize(fn:substring-before($desc, " is not currently in use."), " "))
...
for $descURI in $descURIs
return fn:distinct-values($descURI)